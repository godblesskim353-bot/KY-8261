...
const EDGE_TARGET = 0.01;
const COMBINED_ASK_CEILING = 0.99;
...
function RiskPanel({ data }: { data?: PolymarketLiveSnapshot }) {
  ...
  const rows = [
    ['Combined ask ceiling', '≤ 99.0¢', formatCents(q?.combinedAsk), q?.combinedAsk !== null && q?.combinedAsk !== undefined && q.combinedAsk <= COMBINED_ASK_CEILING],
    ['Minimum pair edge', '≥ 1.0¢', q?.edge === null || q?.edge === undefined ? '—' : `${formatSigned(q.edge * 100, 2)}¢`, q?.edge !== null && q?.edge !== undefined && q.edge >= EDGE_TARGET],
...
function ExecutionPanel({ data }: { data?: PolymarketLiveSnapshot }) {
  ...
  const tone = halted ? 'coral' : autoRunning ? 'teal' : 'amber';
  const busy = arm.isPending || pause.isPending;
  const toggleLabel = autoRunning ? 'PAUSE AUTO EXECUTION' : halted ? 'RESET & START AUTO EXECUTION' : execution?.enabled ? 'START AUTO EXECUTION' : 'SERVER MASTER SWITCH OFF';
  const toggleClick = () => autoRunning ? pause.mutate(undefined) : arm.mutate(undefined);
  const recoveryMessage =
    execution?.state === 'RECOVERING'
      ? 'SINGLE-LEG RECOVERY: trying the +2¢ merge hedge, then the -10% FAK stop.'
      : execution?.state === 'MERGING'
        ? 'COMPLETE SET ACQUIRED: waiting for the exact-size Merge confirmation.'
        : execution?.unresolvedLeg
          ? 'RECOVERY LOCKED: an ambiguous or partial result requires operator review and manual re-arm.'
          : 'Only FOK rejections proven to have zero fills may cool down; exposure never auto-retries.';
  return <section className="panel rounded-xl p-5"><SectionHeader eyebrow="server-side execution" title="Two-leg FOK controller" action={<Badge tone={tone} pulse={autoRunning}>{halted ? 'HALTED' : autoRunning ? 'AUTO 24/7' : execution?.state ?? 'loading'}</Badge>} /><div className="rounded-lg border border-[#e1e4de] bg-[#f7f8f4] p-3"><div className="flex items-start justify-between gap-3"><div><div className="eyebrow text-[#7d8785]">lifecycle reason</div><p className="mt-1 text-[11px] leading-4 text-[#43505a]">{execution?.reason ?? 'Press START to begin automatic execution.'}</p></div><ShieldCheck size={18} className={halted ? 'text-[#c95d51]' : 'text-[#16826c]'} /></div><div className="mt-3 grid grid-cols-2 gap-2 border-t border-[#e1e4de] pt-3 text-[10px]"><div><div className="text-[#7d8785]">planned shares</div><div className="mt-1 font-mono font-semibold text-[#34474a]">{formatShares(execution?.plannedShares)}</div></div><div><div className="text-[#7d8785]">planned cost</div><div className="mt-1 font-mono font-semibold text-[#34474a]">{formatUsd(execution?.plannedCostPusd)}</div></div><div><div className="text-[#7d8785]">YES order</div><div className="mt-1 truncate font-mono text-[#34474a]">{execution?.yesOrderId ? `${execution.yesOrderId.slice(0, 9)}…` : '—'}</div></div><div><div className="text-[#7d8785]">NO order</div><div className="mt-1 truncate font-mono text-[#34474a]">{execution?.noOrderId ? `${execution.noOrderId.slice(0, 9)}…` : '—'}</div></div></div></div><div className={`mt-3 rounded-lg border px-3 py-2 text-[10px] ${execution?.unresolvedLeg ? 'border-[#f0b2aa] bg-[#ffe0dc] text-[#9e342b]' : 'border-[#d6e7df] bg-[#eaf6f1] text-[#387163]'}`}>{recoveryMessage}</div><button type="button" onClick={toggleClick} disabled={busy || (!execution?.enabled && !autoRunning) || (halted && Boolean(execution?.unresolvedLeg))} className={`mt-3 w-full rounded-lg px-3 py-3 text-[11px] font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${autoRunning ? 'border border-[#d9c28a] bg-[#fff8e7] text-[#8d661e] hover:bg-[#ffefc6]' : 'border border-[#16826c] bg-[#e8f6f0] text-[#176856] hover:bg-[#d6f0e5]'}`}>{busy ? 'UPDATING…' : toggleLabel}</button></section>;
}
...
function Dashboard() {
  ...
  return <div className="grain app-shell min-h-[100dvh]"><NavRail executionState={live?.execution.state} /><TopBar data={live} /><main id="overview" className="mx-auto max-w-[1520px] px-4 pb-10 pt-5 md:ml-[224px] md:px-7 md:pt-7"><div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><div className="eyebrow text-[#89938f]">SERVER-OBSERVED / {timestampLabel(live?.serverTime)} UTC</div><h2 className="mt-1 font-display text-[23px] font-semibold tracking-[-.045em] text-[#24343a]">BTC binary market monitor</h2></div><div className="flex items-center gap-2 text-[10px] text-[#7d8884]"><Activity size={13} /><span>{loadingLive ? 'Fetching live snapshot' : `snapshot ${live?.sequence ?? '—'}`}</span><span className={`h-1.5 w-1.5 rounded-full ${live?.ready ? 'bg-[#16826c] live-pulse' : 'bg-[#b9811f]'}`} /></div></div><div className="appear"><StatusHero data={live} now={now} /></div><div className="appear-2 mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Current edge" value={edge === null || edge === undefined ? '—' : `${formatSigned(edge * 100, 2)}¢`} sub="100¢ − combined ask" Icon={Target} tone={edge !== null && edge !== undefined && edge >= EDGE_TARGET ? 'teal' : 'coral'} trend={trend} /><MetricCard label="BTC spot" value={formatUsd(live?.spot.priceUsd, 0)} sub="Binance BTCUSDT stream" Icon={TrendingUp} tone="teal" trend={live?.spot.change60sPct !== null && live?.spot.change60sPct !== undefined && live.spot.change60sPct < 0 ? 'down' : 'up'} /><MetricCard label="Common depth" value={formatShares(live?.quotes.commonDepth)} sub="min(YES ask size, NO ask size)" Icon={Layers3} tone="amber" /><MetricCard label="Net inventory" value={formatSigned(live?.inventory.netShares, 2)} sub={live?.inventory.source ?? 'awaiting source'} Icon={BarChart3} tone="teal" /></div><div className="appear-3 mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.48fr)_minmax(340px,.92fr)]"><div className="space-y-5"><MarketPanel data={live} /><SpotPanel data={live} now={now} /><OpportunityLogPanel /><LiveEvents data={live} /></div><div className="space-y-5"><ExecutionPanel data={live} /><RiskPanel data={live} /><InventoryPanel data={live} /><CompoundPanel data={live} /><ProxyPanel data={live} loading={loadingConnection || !connection} /></div></div><footer className="mt-8 flex flex-col justify-between gap-2 border-t border-[#dbded9] pt-4 text-[10px] text-[#8a9591] sm:flex-row"><span className="flex items-center gap-1.5"><ShieldCheck size={12} />SERVER-SIDE CLOB TWO-LEG FOK · residential proxy enforced · kill switch available</span><span className="font-mono">no browser-side signing or secrets</span></footer></main></div>;
}
