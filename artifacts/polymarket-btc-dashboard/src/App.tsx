...
function DirectionPanel({ data }: { data?: PolymarketLiveSnapshot }) {
  ...
  return <section className="panel rounded-xl p-5">
    <SectionHeader eyebrow="side selector / confirmation" title="BTC + CLOB direction" action={<Badge tone={confirmed ? 'teal' : 'amber'} pulse={confirmed}>{confirmed ? `${direction} confirmed` : 'skip entry'}</Badge>} />
    <div className="grid grid-cols-3 gap-2">
      <div className="rounded-lg border border-[#e1e4de] bg-[#f7f8f4] p-3"><div className="eyebrow text-[#8c9793]">BTC direction</div><div className="mt-2 font-mono text-lg font-semibold text-[#34474a]">{signal?.btcDirection ?? 'NEUTRAL'}</div></div>
      <div className="rounded-lg border border-[#e1e4de] bg-[#f7f8f4] p-3"><div className="eyebrow text-[#8c9793]">book pressure</div><div className="mt-2 font-mono text-lg font-semibold text-[#34474a]">{signal?.bookDirection ?? 'UNCLEAR'}</div></div>
    ...
    <p className="mt-3 text-[10px] leading-4 text-[#70817b]">{signal?.reason ?? 'Waiting for both signal sources.'}</p>
  </section>;
}
  ...
  return <section className="panel rounded-xl p-5">
    ...
    <div className="rounded-lg border border-[#e1e4de] bg-[#f7f8f4] p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 border-t border-[#e1e4de] pt-3 text-[10px]">
        ...
        <div><div className="text-[#7d8785]">entry / target</div><div className="mt-1 font-mono font-semibold text-[#34474a]">{formatCents(execution?.entryPricePusd)} → {formatCents(execution?.takeProfitPricePusd)}</div></div>
    ...
    <div className={`mt-3 rounded-lg border px-3 py-2 text-[10px] ${execution?.lastExitError ? 'border-[#f0b2aa] bg-[#ffe0dc] text-[#9e342b]' : 'border-[#d6e7df] bg-[#eaf6f1] text-[#387163]'}`}>
      {execution?.lastExitError ?? 'Entry requires combined ask <99¢ plus matching BTC and CLOB direction. Exit target is +2¢ per share; incomplete exits retry without HALTED.'}
    </div>
